...
step2A_models = c('NN_reducedV3.2_removeN5_nfeatures21_testsetEval', 
                  'NN_reducedV3.2_removeN5_no.arms_nfeatures21_testsetEval',
                  'NN_reducedV3.2_removeN5_no.focals_nfeatures21_testsetEval', 
                  'NN_reducedV3.2_removeN5_no.scnas_nfeatures21_testsetEval',
                  'NN_reducedV3.2_removeN5_no.sv_nfeatures21_testsetEval',
                  'NN_reducedV3.2_removeN5_no.muts_nfeatures21_testsetEval')

current_table = performancetable[step2A_models, ]
current_table['NN_reducedV3.2_removeN5_nfeatures21_testsetEval', 'Model'] = 'Final Model'
...
rownames(current_table) = gsub('no.', 'no_', rownames(current_table))

current_table$experiment = rownames(current_table)
current_table$experiment <- factor(current_table$experiment, levels = current_table$experiment[order(current_table$Performance)])

current_colors = c("#E3140F", "#000000")
output_fn = 'plots/test_set/combined_performance_step2B_testset'
widths = c(2.5,1,1)
...
ggsave(paste(output_fn, '.jpeg', sep=''), p4, width=11, height=8 * 6/20)
ggsave(paste(output_fn, '.pdf', sep=''), p4, width=11, height=8 * 6/20)