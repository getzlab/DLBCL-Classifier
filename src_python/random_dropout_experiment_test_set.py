...
fullPath = '../saved_models/FINALMODEL*/*'
...
# Mask strategy still works because each roll for each cell is independent
# Numpy shape parameter (third) is just some # of coin flips (total rows * cols amount) arranged in a specific shape.

masks = []
for step in steps:
    mask = np.random.binomial(1, step, (len(sig_genes), len(test_set)))
    masks.append(mask)
...
for step in steps:
    ...
    if step < 0.10:
        ...
        df = df + random_events
    if TEST_MODE:
        datafile = '../random_dropout_experiment/tmpRandomDroppedGSM_fullfeatures_test_step' + str(1-step) + '_testset.txt'
        maskfile = '../random_dropout_experiment/mask_test_step' + str(1-step) + '_testset.txt'
        mask = pd.DataFrame(mask)
        mask.to_csv(maskfile, sep='\t')
    else:
    ...
    if TEST_MODE:
        predictionsDF.to_csv('../random_dropout_experiment/predictions_raw_test_step' + str(1-step) + '_testset.txt')
    if not pMax:
    ...
    if TEST_MODE:
        predictionsDF.to_csv('../random_dropout_experiment/predictions_scaled_test_step' + str(1-step) + '_testset.txt')
...
results_df.to_csv('../random_dropout_experiment/resultstable_random_dropout_testset.txt', sep='\t', index=False)
